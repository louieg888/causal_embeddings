...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Any:
        ...
        if self.dimensions == 2:
            x_tmp = nn.functional.adaptive_avg_pool2d(x, (1))
        if self.dimensions == 3:
            x_tmp = nn.functional.adaptive_avg_pool3d(x, (1))
        x_out = torch.flatten(x_tmp, start_dim=1)
        x_emb = self.intermediate_linear(x_out)

        x = self.decode(x)
        return x, x_emb

if __name__ == '__main__':
    smol_net = AutoEncoder(
        spatial_dims=2,
        in_channels=1,
        out_channels=1,
        channels=(32, 64, 128),
